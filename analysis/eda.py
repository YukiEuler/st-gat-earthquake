...
def plot_preprocessing_stages(df, output_dir):
    """
    Visualize preprocessing stages - simplified to show cropping process only.
    """
    ...
    # Create single figure showing cropping process
    fig, ax = plt.subplots(figsize=(10, 8))
    
    # Plot removed events (gray)
    if len(df_removed) > 0:
        ax.scatter(df_removed['lon'], df_removed['lat'], c='lightgray', alpha=0.3, s=2, label=f'Dihapus: {n_removed:,}')
    
    # Plot retained events (blue)
    ax.scatter(df_cropped['lon'], df_cropped['lat'], c='steelblue', alpha=0.5, s=2, label=f'Dipertahankan: {n_cropped:,}')
    ...
    ax.set_title(f'Proses Cropping\n({n_cropped:,} dari {n_total:,} event dipertahankan, {n_cropped/n_total*100:.1f}%)')
...
def generate_crop_sensitivity_analysis(df, output_dir):
    ...
    # Create single visualization - Density vs Scale only
    fig, ax = plt.subplots(figsize=(10, 6))
    
    scales = [float(r['Scale'].replace('x', '')) for r in results]
    densities = [r['Density'] for r in results]
    
    ax.plot(scales, densities, 'o-', color='purple', linewidth=2, markersize=10)
    ax.axvline(1.0, color='red', linestyle='--', alpha=0.7, linewidth=2, label='Crop Saat Ini')
    ax.set_xlabel('Faktor Skala', fontsize=12)
    ax.set_ylabel('Densitas Event (event/km²)', fontsize=12)
    ax.set_title('Densitas Event vs Skala Crop', fontsize=14, fontweight='bold')
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    
    # Add value annotations
    for i, (s, d) in enumerate(zip(scales, densities)):
        ax.annotate(f'{d:.2f}', (s, d), textcoords="offset points", xytext=(0, 10), 
...
def generate_grid_size_analysis(df, output_dir):
    ...
    # Create single visualization - Qualified Nodes vs Grid Size only
    fig, ax = plt.subplots(figsize=(10, 6))
    
    grids = [r['Grid Size (°)'] for r in results]
    qualified = [r['Qualified Nodes'] for r in results]
    
    ax.plot(grids, qualified, 'o-', color='steelblue', linewidth=2, markersize=10)
    ax.axvline(current_grid, color='red', linestyle='--', alpha=0.7, linewidth=2, label=f'Saat Ini ({current_grid}°)')
    ax.set_xlabel('Ukuran Grid (°)', fontsize=12)
    ax.set_ylabel('Jumlah Node Qualified', fontsize=12)
    ax.set_title(f'Node Qualified vs Ukuran Grid\n(min_events={min_events})', fontsize=14, fontweight='bold')
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    
    # Add value annotations
    for i, (g, q) in enumerate(zip(grids, qualified)):
        ax.annotate(f'{q}', (g, q), textcoords="offset points", xytext=(0, 10), 
                   ha='center', fontsize=9)
...
def plot_grid_and_graph(df_cropped, output_dir):
    ...
    # Create figure with 2 panels (simplified)
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    
    # ===== Panel 1: Grid Structure & Active Nodes (combined) =====
    ax1 = axes[0]
    ...
    # Plot heatmap
    im = ax1.imshow(count_matrix, origin='lower', cmap='YlOrRd',
                    extent=[lon_min, lon_max, lat_min, lat_max],
                    aspect='auto', alpha=0.7)
    plt.colorbar(im, ax=ax1, label='Jumlah Event', shrink=0.8)
    
    # Highlight active nodes
    for node_id in active_nodes:
        lon, lat = node_coords[node_id]
        ax1.plot(lon, lat, 'ko', markersize=4, alpha=0.7)
    
    ax1.set_xlim(lon_min - 0.01, lon_max + 0.01)
    ax1.set_ylim(lat_min - 0.01, lat_max + 0.01)
    ax1.set_xlabel('Bujur')
    ax1.set_ylabel('Lintang')
    ax1.set_title(f'Struktur Grid & Filter Node Aktif\n({n_rows}×{n_cols} sel, {n_active} node aktif ≥ {min_events} event)')
    ...
    # Convert lat/lon to approximate km
    lat_km = 111.0
    lon_km = 111.0 * np.cos(np.radians(np.mean(coord_array[:, 1])))
    ...
    # Draw edges
    nx.draw_networkx_edges(G, pos, ax=ax2, alpha=0.4, edge_color='gray', width=0.8)
    ...
    nodes = nx.draw_networkx_nodes(G, pos, ax=ax2, node_size=60, 
